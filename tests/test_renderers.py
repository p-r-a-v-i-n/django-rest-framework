...
class UnicodeJSONRendererTests(TestCase):
    ...
    def test_proper_encoding(self):
        ...
        content = renderer.render(obj, "application/json")
        self.assertEqual(
            content, '{"countries":["United Kingdom","France","España"]}'.encode()
        )
...
class AsciiJSONRendererTests(TestCase):
    ...
    def test_proper_encoding(self):
        ...
        content = renderer.render(obj, "application/json")
        self.assertEqual(
            content, b'{"countries":["United Kingdom","France","Espa\\u00f1a"]}'
        )
...
class TestDateTimeFieldHTMLFormRender(TestCase):
    ...
    def _assert_datetime_rendering(
        self, appointment, expected, datetimefield_kwargs=None
    ):
    ...
    def test_datetime_field_rendering_with_custom_datetime_format(self):
        self._assert_datetime_rendering(
            datetime(2024, 12, 24, 0, 55, 30, 345678), "2024-12-24T00:55:00.000"
        )
    ...
    def test_datetime_field_rendering_timezone_aware_datetime(self):
        self._assert_datetime_rendering(
            datetime(
                2024, 12, 24, 0, 55, 30, 345678, tzinfo=ZoneInfo("Asia/Tokyo")
            ),  # +09:00
...
class BrowsableAPIRendererTests(URLPatternsTestCase):
    ...
    def test_render_form_for_serializer(self):
        with self.subTest("Serializer"):
            serializer = BrowsableAPIRendererTests.SimpleSerializer(
                data={"name": "Name"}
            )
        ...
        with self.subTest("ListSerializer"):
            list_serializer = BrowsableAPIRendererTests.SimpleSerializer(
                data=[{"name": "Name"}], many=True
            )
    ...
    def test_get_raw_data_form(self):
        with self.subTest("Serializer"):
            ...
            raw_data_form = self.renderer.get_raw_data_form(
                {"name": "Name"}, view, "POST", request
            )
        ...
        with self.subTest("ListSerializer"):
            ...
            class DummyGenericViewsetLike(APIView):
                def get_serializer(self, **kwargs):
                    return BrowsableAPIRendererTests.SimpleSerializer(
                        many=True, **kwargs
            ...
            assert (
                raw_data_form["_content"].initial
                == '[\n    {\n        "name": ""\n    }\n]'
            )
