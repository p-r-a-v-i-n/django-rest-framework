...
class TestUniquenessValidation(TestCase):
    ...
    def test_relation_is_not_unique(self):
        ...
        assert not serializer.is_valid()
        assert serializer.errors == {
            "user": ["related model with this user already exists."]
        }
    ...
    def test_doesnt_pollute_model(self):
        ...
        assert all(
            [
                "Unique" not in repr(v)
                for v in AnotherUniquenessModel._meta.get_field("code").validators
            ]
        )
...
class TestUniquenessTogetherValidation(TestCase):
    ...
    def test_read_only_fields_with_default_and_source(self):
        class ReadOnlySerializer(serializers.ModelSerializer):
            name = serializers.CharField(
                source="race_name", default="test", read_only=True
            )
    ...
    def test_ignore_validation_for_unchanged_fields(self):
        ...
        with patch("rest_framework.validators.qs_exists") as mock:
            assert serializer.is_valid()
...
class TestUniqueConstraintValidation(TestCase):
    def setUp(self):
        self.instance = UniqueConstraintModel.objects.create(
            race_name="example", position=1, global_id=1, fancy_conditions=1
        )
        UniqueConstraintModel.objects.create(
            race_name="example", position=2, global_id=2, fancy_conditions=1
        )
        UniqueConstraintModel.objects.create(
            race_name="other", position=1, global_id=3, fancy_conditions=1
        )
    ...
    def test_single_field_uniq_validators(self):
        ...
        ids_in_qs = {
            frozenset(v.queryset.values_list(flat=True))
            for v in validators
            if hasattr(v, "queryset")
        }
    ...
    def test_uniq_constraint_condition_read_only_partial(self):
        ...
        instance = UniqueConstraintReadOnlyFieldModel.objects.create(
            position=1, something=1
        )
        serializer = UniqueConstraintReadOnlyFieldModelSerializer(
            instance=instance, data={"position": 1, "something": 1}, partial=True
        )
        assert serializer.is_valid()

    def test_unique_constraint_custom_message_code(self):
        UniqueConstraintCustomMessageCodeModel.objects.create(
            username="Alice", company_id=1, role="member"
        )
        expected_code = (
            "duplicate_username"
            if django_version[0] >= 5
            else UniqueTogetherValidator.code
        )

        serializer = UniqueConstraintCustomMessageCodeSerializer(
            data={
                "username": "Alice",
                "company_id": 1,
                "role": "admin",
        ...
        assert not serializer.is_valid()
        assert serializer.errors == {
            "non_field_errors": ["Username must be unique within a company."]
        }
    ...
    def test_unique_constraint_default_message_code(self):
        ...
        expected_message = UniqueTogetherValidator.message.format(
            field_names=", ".join(("company_id", "role"))
        )
        assert not serializer.is_valid()
        assert serializer.errors == {"non_field_errors": [expected_message]}
        assert (
            serializer.errors["non_field_errors"][0].code
            == UniqueTogetherValidator.code
        )
...
class ValidatorsTests(TestCase):
    ...
    def test_equality_operator(self):
        ...
        validator = BaseUniqueForValidator(
            queryset=mock_queryset, field="foo", date_field="bar"
        )
        validator2 = BaseUniqueForValidator(
            queryset=mock_queryset, field="foo", date_field="bar"
        )
