...
class SearchSplitTests(SimpleTestCase):
    ...
    def test_strips_commas_around_quoted(self):
        assert ["hello, world"] == list(filters.search_smart_split(',,"hello, world"'))
        assert ["hello, world"] == list(
            filters.search_smart_split(',,"hello, world",,')
        )
        assert ["hello, world"] == list(filters.search_smart_split('"hello, world",,'))
    ...
    def test_splits_quotes_followed_by_comma_and_sentence(self):
        assert ['"hello', 'world"', "found"] == list(
            filters.search_smart_split('"hello, world",found')
        )
...
class SearchFilterFkTests(TestCase):
    ...
    def test_custom_lookup_to_related_model(self):
        ...
        assert "attribute__label__icontains" == filter_.construct_search(
            "attribute__label", SearchFilterModelFk._meta
        )
        assert "attribute__label__iendswith" == filter_.construct_search(
...
class OrderingFilterRelatedModel(models.Model):
    related_object = models.ForeignKey(
        OrderingFilterModel, related_name="related", on_delete=models.CASCADE
    )
...
class OrderingFilterSerializerWithModelProperty(serializers.ModelSerializer):
    class Meta:
        model = OrderingFilterModel
        fields = ("id", "title", "text", "description")
...
class OrderingFilterTests(TestCase):
    ...
    def test_ordering_by_aggregate_field(self):
        ...
        num_objs = [2, 5, 3]
        for obj, num_related in zip(OrderingFilterModel.objects.all(), num_objs):
