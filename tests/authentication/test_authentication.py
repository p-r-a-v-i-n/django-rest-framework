...
urlpatterns = [
    path("session/", MockView.as_view(authentication_classes=[SessionAuthentication])),
    path("basic/", MockView.as_view(authentication_classes=[BasicAuthentication])),
    path(
        "remote-user/",
        MockView.as_view(authentication_classes=[RemoteUserAuthentication]),
    ),
    path("token/", MockView.as_view(authentication_classes=[TokenAuthentication])),
    path(
        "customtoken/",
...
class TokenAuthTests(BaseTokenAuthTests, TestCase):
    ...
    def test_generate_key_randomness_quality(self):
        ...
        for key in keys:
            ...
            assert not all(c == key[0] for c in key), (
                f"Key has all same characters: {key}"
            )
