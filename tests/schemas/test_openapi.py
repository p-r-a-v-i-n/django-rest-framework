...
class TestFieldMapping(TestCase):
        ...
        cases = [
            ...
            (
                serializers.ListField(
                    child=serializers.IntegerField(max_value=4294967295)
                ),
                {
                    "items": {
                        "type": "integer",
            ...
            (
                serializers.ListField(
                    child=serializers.ChoiceField(choices=[(1, "One"), (2, "Two")])
                ),
            ...
            (
                serializers.ListField(
                    child=serializers.ChoiceField(
                        choices=[(uuid1, "uuid1"), (uuid2, "uuid2")]
                    )
                ),
    ...
    def test_lazy_string_field(self):
        ...
        assert isinstance(data["properties"]["text"]["description"], str), (
    ...
    def test_boolean_default_field(self):
        ...
        assert data["properties"]["default_true"]["default"] is True, (
            "default must be true"
        )
        assert data["properties"]["default_false"]["default"] is False, (
            "default must be false"
        )
        assert "default" not in data["properties"]["without_default"], (
    ...
    def test_nullable_fields(self):
        ...
        data = inspector.map_serializer(Serializer())
        assert data["properties"]["rw_field"]["nullable"], (
            "rw_field nullable must be true"
        )
        assert data["properties"]["ro_field"]["nullable"], (
            "ro_field nullable must be true"
        )
        assert data["properties"]["ro_field"]["readOnly"], (
            "ro_field read_only must be true"
        )

    def test_primary_key_related_field(self):
        class PrimaryKeyRelatedFieldSerializer(serializers.Serializer):
            basic = serializers.PrimaryKeyRelatedField(
                queryset=BasicModel.objects.all()
            )
            uuid = serializers.PrimaryKeyRelatedField(
                queryset=BasicModel.objects.all(), pk_field=serializers.UUIDField()
            )
            char = serializers.PrimaryKeyRelatedField(
                queryset=BasicModel.objects.all(), pk_field=serializers.CharField()
            )
...
class TestOperationIntrospection(TestCase):
    def test_path_without_parameters(self):
        ...
        view = create_view(views.DocStringExampleListView, method, create_request(path))
        inspector = AutoSchema()
        inspector.view = view
    ...
    def test_path_with_id_parameter(self):
        ...
        view = create_view(
            views.DocStringExampleDetailView, method, create_request(path)
        )
        inspector = AutoSchema()
        inspector.view = view
    ...
    def test_auto_generated_apiview_tags(self):
        ...
        generator = SchemaGenerator(patterns=url_patterns)
        schema = generator.get_schema(request=create_request("/"))
        assert schema["paths"]["/any-dash_underscore/"]["get"]["tags"] == [
            "any-dash-underscore"
        ]
        assert schema["paths"]["/restaurants/branches/"]["get"]["tags"] == [
            "restaurants"
        ]
...
class TestGenerator(TestCase):
    ...
    def test_authtoken_serializer(self):
        patterns = [path("api-token-auth/", obtain_auth_token)]
        generator = SchemaGenerator(patterns=patterns)
        ...
        assert body_schema == {"$ref": "#/components/schemas/AuthToken"}
    ...
    def test_component_should_not_be_generated_for_delete_method(self):
        ...
        assert (
            "content" not in schema["paths"]["/example/"]["delete"]["responses"]["204"]
        )
